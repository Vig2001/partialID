"""
Coverage analysis of sample splitting + optimisation method for fused ID set (Zhao, Small, Bhattacharya 2022)
The idea is to split the data into two folds, A and B.
Fold A is used to find the optimal weight (omega_L, omega_U) that minimises the width of the fused ID set.
Fold B is used to construct a valid confidence interval using the frozen weights from Fold A.

We compare coverage with the above method and the naive method with no sample splitting.
The coverage target is the true fused partial ID band, which we approximate using a large sample (N=1,000,000) from the DGP.

There is a horowitz-manski style CI construction that is used to construct a valid CI for the fused ID set.
"""
...
def run():
    ...
    for m in range(M_MC):
        ...
        # Asymptotic normality check (optional)
        # if m == 0:
